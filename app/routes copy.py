...
def checkout():
    ...
    if form.validate_on_submit():
        ...
        # ✅ Calculate transactions in the last 24 hours
        ...
        sample["hour_sin"] = np.sin(2 * np.pi * sample["transaction_hour"] / 24)
        sample["hour_cos"] = np.cos(2 * np.pi * sample["transaction_hour"] / 24)

        sample_df = pd.DataFrame([sample])
        sample_processed = preprocessor.transform(sample_df)

        xgb_prob = xgb_model.predict_proba(sample_processed)[0][1]
        iso_score = iso_model.decision_function(sample_processed)[0]
